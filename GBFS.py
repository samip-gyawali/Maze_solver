class Node(object):
    def __init__(self,state,parent,action,goalState):
        ...
        self.parent = parent
        self.action = action
        self.heuristic = abs(goalState[0]-self.state[0]) + abs(goalState[1]-self.state[1]) # Manhattan Distance
...
class Frontier(object):
    ...
    def add(self,node):
        ...
        while i < len(self.frontier) and not found:
            if self.frontier[i].heuristic < node.heuristic:
...
class Maze(object):
    ...
    def __showSolution(self):
        ...
        with open('./GBFS/Maze_3/solution.txt','w') as f:
            for r in range(self.height):
                for c in range(self.width):
                    if (r,c) == self.start:
                        f.write('A')
    ...
    def __showExplored(self):
        ...
        with open('./GBFS/Maze_3/explored.txt','w') as f:
            for r in range(self.height):
                for c in range(self.width):
                    if (r,c) == self.start:
                        f.write('A')
